...
def calculate_strategy(df):
    ...
    for i in range(1, len(df)):
        ...
        if current_position == 1:
            ...
            # Stop-loss for long: if low <= 0.985 * prev_price, capital * 0.985
            if df['low'].iloc[i] <= 0.985 * prev_price:
                capital = prev_capital * 0.985
            ...
            # Stop-loss for short: if high >= 1.015 * prev_price, capital * 0.985
            if df['high'].iloc[i] >= 1.015 * prev_price:
                capital = prev_capital * 0.985
                df.loc[df.index[i], 'stop_loss_triggered'] = True
        else:
